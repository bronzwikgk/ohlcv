...
markdown_cell = nbf.v4.new_markdown_cell("""# Micro-Pullback Strategy Backtester (Sample & Visualizer)
...
OUTPUT_HTML = r'output/pullback_sample_report.html'
""",
    """# 2. Indicator Math
...
def calculate_all_indicators(df):
    ...
    # But we return only the last 6 months (approx 126 trading days) for the backtest phase
    df = df.dropna().reset_index(drop=True)
    if len(df) > 126:
        df = df.tail(126).reset_index(drop=True)
    return df
""",
    """# 3. Backtest Engine
...
print(f"Processing {len(sample_files)} stocks over 6-month period...")
...
nb.cells = [markdown_cell] + [nbf.v4.new_code_cell(c) for c in code_cells]

output_path = "notebook/ohlcv_pullback_strategy.ipynb"
