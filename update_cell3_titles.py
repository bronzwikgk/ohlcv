...
new_code = """# Cell 3: Extracting and Saving JPEGs (With Seed & Rules)
BLOCK_SIZE = 21
...
def save_25_samples_rulebased(block_list, title, filename_prefix):
    ...
    # Grid Title
    full_title = f"{title} (Seed: {SEED}) | Filter: 14d Ret >= 3.0% | Zones: X_Mid={x_mid:.2f}%, Y_Mid={y_mid:.2f}%"
    ...
    plt.suptitle(full_title, fontsize=16, y=1.01)
    plt.tight_layout()
    grid_filename = f'../output/{filename_prefix}_seed{SEED}_grid.jpg'
    plt.savefig(grid_filename, format='jpeg', bbox_inches='tight')
    plt.close(fig)
    ...
    plt.title(f'Rule-Based Zone Scatter Plot | {full_title}', fontsize=14)
    ...
    scatter_filename = f'../output/{filename_prefix}_seed{SEED}_scatter.jpg'
    plt.savefig(scatter_filename, format='jpeg', bbox_inches='tight')
    plt.close()
    
    # Display images natively in jupyter
    display(Image(filename=grid_filename))
...
# If there's an extra markdown cell at the end (from my previous run), remove it
