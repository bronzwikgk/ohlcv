...
def plot_25_samples_clustered(block_list, title):
    ...
    # ---> SORT THE 25 SAMPLES BY CLUSTER LABEL BEFORE PLOTTING <---
    bundled = list(zip(sample_blocks, returns, atrs, labels_text, cluster_labels))
    bundled.sort(key=lambda x: x[4]) # Sort by cluster ID
    sample_blocks, returns, atrs, labels_text, cluster_labels = zip(*bundled)
    
    # Color map for the 4 clusters
    colors = ['#ef4444', '#3b82f6', '#22c55e', '#f59e0b']
    
    # 1) Grid Plot with Price only (fast render)
    ...
    plt.suptitle(f"{title} - Price Panels (Sorted by K-Means Cluster)", fontsize=18, y=1.01)
    plt.tight_layout()
    plt.show()
